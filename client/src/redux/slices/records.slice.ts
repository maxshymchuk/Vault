...
const filteredRecordsSelector = createSelector(
  [
    ...
    (state: RootState) => state.records.search,
  ],
  (records, search) => {
...
function filterRecords(
  records: Array<VaultRecord>,
  recordsToRemove: Array<VaultRecord> | VaultRecord
) {
  if (Array.isArray(recordsToRemove)) {
    return records.filter(
      (record) => !recordsToRemove.map((r) => r.id).includes(record.id)
    );
  } else {
...
const recordsSlice = createSlice({
  ...
  reducers: {
    ...
    selectRecords: (
      state,
      { payload }: PayloadAction<Array<VaultRecord> | VaultRecord>
    ) => {
      if (Array.isArray(payload)) {
        ...
        if (index === -1) {
          ...
          state.selectedRecords = state.selectedRecords.filter(
            (record) => record.id !== payload.id
          );
        }
      }
